	...
	const handleMessage=(e)=>{
      ...
      const {image} = UserMessage
		...
		const time = new Date(Date.now()).getHours()+':'+new Date(Date.now()).getMinutes()
				socketRef.current.emit('message',{time,name,image,message,privateId})
		e.preventDefault()

	}
const handleID=(e)=>{
	if (privateId==null){
setPrivateId(e.target.className)
}else{
	setPrivateId(null)
}
...
{true&&<Member dataUser={dataUser} notify={notify} handleID={handleID}/>}
